...
function isadv() {
   ...
   {
     tt=parent.frames[2].name;
     if (( tt == 'basefrm') || ( tt == 'mainFrame'))
        ia = 1 ;
   }   
   return ia;
}

function isValidVLAN(val){
	if(val == "")
		return false;
	if(isNaN(val))
		return false;
	if(parseInt(val) < 1 || parseInt(val) > 4089)
		return false;
	return true;	
}
